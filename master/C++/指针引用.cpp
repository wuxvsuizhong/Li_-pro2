...
/* 指针引用和普通引用的区别在于，它引用的对象是一个指针，而不是一个普通的类型变量 */
...
int main()
{
	...
	int num2(10);
	int *pnum(&num1);
	int * & rpnum=pnum;//指针的引用（引用的是一个int类型的指针）,此处rpnum引用的是指针pnum,而pnum又指向num1
	cout<<*pnum<<endl;
	//cout<<num1<<endl;
	rpnum = &num2;//此处被rpnum引用的指针pnum重新赋值指向num2,即通过指针的引用改变指针的值 
	cout<<*pnum<<endl;
	//cout<<num2<<endl;
	...
	*rpnum = 1111;//通过引用改变指针指向的值
	...
	//int * & rrpnum = &num1; //对于num1取地址的操作为一个右值（表达式)，不能直接引用(语法报错)
	//int * && rrpnum = &num1;  //所以需要使用右值引用 &&

//	int *gpp;//如果采用先分配变量，然后赋值的做法，必须提前分配用于保存栈指针的引用的变量，因为如果在函数调用返回后再分配栈变量,那么栈会被刷
	int * &gp = getp();//指针引用(引用getp函数返回的栈上的指针)
	int *gpp = gp;//采用即时分配变量并赋值的方式，保存栈指针的内容，
	// 其实可以简单的把这里的引用就看做是目标变量，然后这里，调用之后用一个新的变量去接受函数中返回的变量。只不过这里特殊的一点是变量是一个指针而已

	cout<<"*gp ="<<*gp<<endl;//在语句执行完成之后，栈被重新刷新重用，gp引用的栈内容变得不可预知
//	cout<<"*gp ="<<*gp<<endl;//再次执行会发生段错误，因为栈已经被刷新,通过指针引用访问分配的对内容已不能实现
	cout<<"gp ="<<gp<<endl;//此处通过引用打印出的栈内容已不是函数调用中分配的堆内存地址
	cout<<"   "<<endl;
	cout<<"gp ="<<gp<<endl;
	cout<<"*gpp = "<<*gpp<<endl;
