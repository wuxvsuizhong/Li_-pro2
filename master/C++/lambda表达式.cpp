...
public:
	...
	{
		int x=3;
		//auto fun = [this,x](int v){cout<<v+this->num<<endl;};//[this,x]访问块语句类的局部变量，有副本机制，无法修改源数据
		auto fun = [&](int v){cout<<v+this->num<<endl;};//[&]引用所有的局部变量外加this指针，无副本机制，可以直接修改源数据
...
int main()
{
	...
	*/
	...
	cout<<func(1,3)<<endl;
	cout<<"-----------\n";
	auto funAdd = [](int a){a *= 2;cout<<a<<endl;};
	...
	int num = 10;
	cout<<"lambda [=]-->"<<endl;
	for_each(myv.begin(),myv.end(),[=](int a){a+=num;cout<<a<<endl;});
	//[=]用于捕获当前块语句中的局部变量num,有副本机制，可以读取变量，但是不能修改源变量
	cout<<endl;

	cout<<"lambda [&num]-->"<<endl;
	for_each(myv.begin(),myv.end(),[&num](int a){a+=num;cout<<a<<endl;num+=1;});
	//[&a】引用变量num,可以读写num
	cout<<endl;

	[](){cout<<"hello world!"<<endl;}();//lambda表达式加上()可以直接调用

	auto function = []()->double{return 6.0/5;};//带返回值的lambda表达式，按照 []()->类型{}  格式
	cout<<function()<<endl;
	auto function2 = [](int a,double b)->decltype(a/b){return a/b;};//decltype自动获取返回值的类型
